...
import Select from '../../../../activity/component/select';
import Calendar from '../../../../activity/component/selectDate';
import TxtInput from '../../../../activity/component/inputList';
...
export default class ActiveRecordEdit extends Component {
    ...
    render() {
        ...
        if (this.state.loading) {
            return (
                <SafeAreaView style={{ flex: 1, backgroundColor: Theme.color2 }} forceInset={{ bottom: 'never' }} >
                    <View style={styles.container}>
                        <NavBar headerOpt={{ back: '活动记录修改', title: '活动记录修改', search: true }} navigation={navigation} />
                        <Loading />
                    </View>
                </SafeAreaView>
            )
            ...
            const { dataSource, comment_field } = this.state;
            // 方案
            let selectList = [];
    ...
    onSubmit() {
        ...
        let url = Api.saveActivityComment;

        let { comment_field } = this.state;
    ...
    getData() {
        ...
        let memberId = signState.r_id;
        let { id } = this.state;
        let url = Api.getActivityComment + '?memberid=' + memberId + '&commentid=' + id;
        fetch(url)
            .then((response) => {
                if (response.ok) {
                    response.json()
