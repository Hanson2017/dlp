...
export default class GongshangBiangengComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            isHidden: [],
    ...
    render() {
        ...
        return (
            <View style={[Theme.box, Theme.mt10]}>
                <Title data={'变更记录'} navigation={navigation} screenUrlInfo={data.length > 5 ? { screenUrl: 'DetailBiangeng', tabId: data } : null} />
                {
                    data.length > 0 ?
                        data.map((item, i) => {
                            if (i < 5) {
